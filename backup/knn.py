...
def tst_rss_crd(f_c_tra, f_c_tst, k, tst_rp, radius_dict):
    ...
    for rp, fp in f_c_tra.items():
        ...
        euclid_dis[rp] = float(Decimal(math.sqrt(temp_dis)).quantize(Decimal('0.000')))
        eraw_dis = euclid_dis
        eraw_dis = OrderedDict(sorted(eraw_dis.items(), key=lambda d: d[1]))
    ...
    visible_file = r'E:\db\108knn_point_visible.csv'
...
def draw_plot_month(myList, Title, Xlabel, Ylabel, No):
    ...
    x1 = range(1, 16, 1)
    plt.figure(int(No))
    plt.plot(x1, y1, label='Error KNN', linewidth=1, color='r', marker='o', markerfacecolor='green', markersize=2)
    plt.xlabel(Xlabel)
    plt.ylabel(Ylabel)
    plt.title(Title)
...
def select_tra_tst(tl):
    ...
    output_file = 'E:\\db\\cdf-r\\knn2m.csv'
...
def main():
    ...
    # 测试序列
    output_merge_file = 'E:\\db\\knn_error\\error_of_tst.csv'
    test_m_end = 2
    test_m_start = 2
    test_tra_no = 1
