...
def rss_crd_rmse(tra_filename):
    ...
    with open(tra_filename) as f:
        ...
        r_len = int(len(reader) / 2)
        fp_len = len(reader[0])
        rss = np.array(list(map(float, reader[0])))
        for i in range(1, r_len):
            rss = np.vstack((rss, list(map(float, reader[i]))))
        # rss[rss == 100] = -105
...
def rss_crd_sa(tra_filename):
    ...
    # for i in range(0, r_len, 6):
    #     fp_coor['rp' + str(i // 6)] = list(map(str, rss[i // 6, :]))
...
def rss_crd_row(tra_filename):
    ...
    with open(tra_filename) as f:
        ...
        fp_len = len(reader[0])
        r_len = len(reader) / 50
        for i in range(int(r_len)):
            fp = [None] * fp_len
            for j in range(fp_len):
                fp[j] = float(reader[i][j])
...
def select_tra_tst(month, test_no, fp_coor_tra, radius_d, floor, row_state):
    ...
    tst_filename = "E:\\db\\simulation-csv\\rss.csv"
    ...
    # 将定位误差写入csv文件
    output_file = 'E:\\db\\cdf2019\\simu\\wknn-simulation.csv'
...
def main():
    ...
    for month in range(test_m_start, test_m_end + 1):
        ...
        filename = "E:\\db\\simulation-csv\\off-rss.csv"
