...
def tst_rss_crd(f_c_tra, f_c_tst, k, tst_rp):
    ...
    visible_file = r'E:\db\wknn_imp_point_visible.csv'
...
def draw_plot_month(myList, Title, Xlabel, Ylabel, No):
    ...
    x1 = range(1, 16, 1)
    plt.figure(int(No))
    plt.plot(x1, y1, label='Error WKNN-IMP', linewidth=1, color='r', marker='o', markerfacecolor='green', markersize=2)
    plt.xlabel(Xlabel)
    plt.ylabel(Ylabel)
    plt.title(Title)
    plt.savefig('WKNN-IMP-Error-75 of month No.%s.png' % No)
...
def select_tra_tst(tl):
    ...
    output_file = 'E:\\db\\make_cdf\\wknn-m2-ori.csv'
