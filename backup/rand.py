...
def tst_rss_crd(f_c_tra, f_c_tst, tst_rp):
    ...
    xcoor = random.random() * 10.8 + 3.1
    ycoor = random.random() * 14.3 + 15.8

    xcoor = float(Decimal(xcoor).quantize(Decimal('0.00')))
    ycoor = float(Decimal(ycoor).quantize(Decimal('0.00')))

    [realx, realy] = f_c_tst[tst_rp][-3:-1]

    vis_temp = [tst_rp, realx, realy, xcoor, ycoor]

    visible_file = r'E:\db\rand_point_visible.csv'
...
def select_tra_tst(tl):
    ...
    # output_file = 'E:\\db\\output_vote\\' + tl[0] + tl[1] + tl[2] + tl[3]
    ...
    output_file = 'E:\\db\\make_cdf\\rand.csv'
...
def main():
    ...
    output_merge_file = 'E:\\db\\rand_error\\rand_error_of_tst.csv'
    # 测试序列
    test_m_end = 15
    test_m_start = 1
    test_tra_no = 1
