...
def get_k(dis):
    ...
    for k in extra.keys():
        if extra[k] <= avg:
            out_k = out_k + 1
    out_k = 4 if out_k > 4 else out_k
    return out_k


@clock
def tst_rss_crd(f_c_tra, f_c_tst, k, tst_rp):
    # euclidean metric
    euclid_dis = {}
    ...
    for rp, fp in f_c_tra.items():
        ...
        for i in range(fp_lens):
            ...
            temp_dis = temp_dis + abs(int(fp[i]) - int(f_c_tst[tst_rp][i]))

        euclid_dis[rp] = float(Decimal(temp_dis).quantize(Decimal('0.00')))
    ...
    visible_file = r'E:\db\wknn_blockdis_point_visible.csv'
...
def draw_plot_month(myList, Title, Xlabel, Ylabel, No):
    ...
    x1 = range(1, 16, 1)
    plt.figure(int(No))
    plt.plot(x1, y1, label='Error WKNN-BLOCKDIS', linewidth=1, color='r', marker='o', markerfacecolor='green', markersize=2)
    plt.xlabel(Xlabel)
    plt.ylabel(Ylabel)
    plt.title(Title)
    plt.savefig('WKNN-BDIS-Error-75 of month No.%s.png' % No)
...
def select_tra_tst(tl):
    ...
    for rp in range(len(fp_coor_tst)):
        if floor == '5':
            if tl[3] != '04':
                e_dis = tst_rss_crd(fp_coor_tra, fp_coor_tst, w_k, 'rp' + str(rp + 48))
            elif tl[3] == '04':
                e_dis = tst_rss_crd(fp_coor_tra, fp_coor_tst, w_k, 'rp' + str(rp + 68))
            error_s = error_s + [e_dis]
        elif floor == '3':
            e_dis = tst_rss_crd(fp_coor_tra, fp_coor_tst, w_k, 'rp' + str(rp))
            error_s = error_s + [e_dis]
    ...
    output_file = 'E:\\db\\make_cdf\\wknn-blockdis.csv'
...
def main():
    ...
    # 测试序列
    output_merge_file = 'E:\\db\\wknn_error\\wknn-blockdis.csv'
    test_m_end = 15
    test_m_start = 1
    test_tra_no = 1
