...
def radius_get(f_c_tra):
    ...
    for k, query in enumerate(f_c_tra):
        ...
        # COS对应-r参数
        temp_up = f_c_tra * out_tra
        sum_up = np.sum(temp_up[:, :448], axis=1)
        stp = np.sqrt(np.sum((out_tra ** 2)[:, :448], axis=1)[0])
        srp = np.sum((f_c_tra ** 2)[:, :448], axis=1)
        sum_down = np.sqrt(srp) * stp
        rp_points = sum_up / sum_down
        rp_points = np.reshape(rp_points, (np.shape(temp_dis)[0], 1))
        # 计算欧氏距离
        ...
        # 排序 选出最小值
        euclid_dis = OrderedDict(sorted(euclid_dis.items(), key=lambda d: d[1], reverse=True))
...
# 指纹匹配方法
def tst_rss_crd(f_c_tra, f_c_tst, tst_rp, AP_r, r_mat):
    """
    :param f_c_tra: 训练集指纹
    :param f_c_tst: 测试集指纹
    :param tst_rp: 待测试参考点
    ...
    # COS相似度
    # max_1 = np.max(f_c_tra[:, :448])
    # min_1 = np.min(f_c_tra[:, :448])
    # f_c_tra[:, :448] = (f_c_tra[:, :448] - min_1) / (max_1 - min_1)
    # Sorensen distance
    # temp_up = f_c_tra * f_tst
    # sum_up = np.sum(temp_up[:, :448], axis=1)
    # stp = np.sqrt(np.sum((f_tst ** 2)[:, :448], axis=1)[0])
    # srp = np.sum((f_c_tra ** 2)[:, :448], axis=1)
    # sum_down = np.sqrt(srp) * stp
    # rp_points = (sum_up / sum_down)

    up_dis = np.abs(f_c_tra - f_tst)
    down_dis = f_c_tra + f_tst
    temp_dis = up_dis / down_dis
    # temp_dis[temp_dis < threshold] = 0
    rp_points = np.sum(temp_dis[:, ap_mat_list], axis=1)
    ...
    rp_vote = OrderedDict(sorted(rp_vote.items(), key=lambda d: d[1], reverse=True))
...
# 实现仿真定位部分
def select_tra_tst(month, test_no, fp_coor_tra, floor, row_state, AP_r, r_mat):
    ...
    for rp in range(np.shape(fp_coor_tst)[0]):
        ...
        if row_state == 0:
            e_dis = tst_rss_crd(fp_coor_tra, fp_coor_tst, 'rp' + str(rp), AP_r, r_mat)
    ...
    # 将定位误差写入csv文件
    output_file = 'E:\\db\\cdf2019\\Sorensen.csv'
...
def main():
    ...
    for month in range(test_m_start, test_m_end + 1):
        ...
        for test_no in trange(1, 6):
            error_75 = select_tra_tst(month, test_no, tra_data, floor, row_state, AP_r, r_mat)
